...
Clone using SSH into a new project names `demo`:
```bash
> git clone  git@github.com:io-tupelo/clj-template.git  demo
```

or, clone using HTTPS (if you don't have SSH set up on GitHub):
```bash
> git clone  https://github.com/io-tupelo/clj-template.git  demo
```
...
~/io.tupelo/clj-template > lein --version
...
```bash
alias lctr="lein do clean, test-refresh"
```
